  ...
  Section GlobalMaps.
    ...
    *)
    ...
    (** Interactive definitions are stored in modtype if not type-annotated.
      Otherwise, the interactive definition is in impl, type annotation in
      modtype. *)
Inductive on_structure_field Σ : structure_field -> Type :=
  | on_sfconst c      : on_constant_decl Σ c
                        -> on_structure_field Σ (sfconst c)
  | on_sfmind kn inds : on_inductive Σ kn inds
                        -> on_structure_field Σ (sfmind inds)
  | on_sfmod mi sb    : on_module_impl Σ mi
                        -> on_structure_body Σ sb
                        -> on_structure_field Σ (sfmod mi sb)
  | on_sfmodtype mtd  : on_structure_body Σ mtd
                        -> on_structure_field Σ (sfmodtype mtd)
...
with on_module_impl Σ : module_implementation -> Type :=
  ...
  | on_mi_struct sb : on_structure_body Σ sb
                      -> on_module_impl Σ (mi_struct sb)
...
Definition on_module_decl Σ m := on_module_impl Σ m.1
                                  × on_module_type_decl Σ m.2.
    ...
    Record on_global_decls_data (univs : ContextSet.t)
      retro (Σ : global_declarations)
      (kn : kername) (d : global_decl) :=
        {
    ...
    Inductive on_global_decls (univs : ContextSet.t) (retro : Retroknowledge.t)
      : global_declarations -> Type :=
    ...
    Definition on_global_env (g : global_env) : Type :=
      on_global_univs g.(universes)
      × on_global_decls g.(universes) g.(retroknowledge) g.(declarations).
  ...
  Proof.
  intros H HP.
  cut ((forall (m : module_implementation),
          on_module_impl Pcmp P Σ m -> on_module_impl Pcmp Q Σ m)
    × (forall (p : structure_field),
          on_structure_field Pcmp P Σ p -> on_structure_field Pcmp Q Σ p)
    × (forall (s : structure_body),
          on_structure_body Pcmp P Σ s -> on_structure_body Pcmp Q Σ s)).
  intro md_sf_sb.
  destruct d; simpl.
  ...
  - apply (on_mi_sf_sb_mutrect); try now constructor.
    -- intros c oncp. constructor.
        now apply on_constant_decl_impl with (Pcmp := Pcmp) (P := P).
    -- intros kn' mind onip. apply on_sfmind with (kn := kn').
        now apply on_inductive_decl_impl with (Pcmp := Pcmp) (P := P).
  Qed.
